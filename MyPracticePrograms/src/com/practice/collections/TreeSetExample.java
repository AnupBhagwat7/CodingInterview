package com.practice.collections;

import java.util.*;

class Book1 implements Comparable<Book1> {
	int id;
	String name, author, publisher;
	int quantity;

	public Book1(int id, String name, String author, String publisher, int quantity) {
		this.id = id;
		this.name = name;
		this.author = author;
		this.publisher = publisher;
		this.quantity = quantity;
	}

	public int compareTo(Book1 b) {
		if (id > b.id) {
			return 1;
		} else if (id < b.id) {
			return -1;
		} else {
			return 0;
		}
	}
}

public class TreeSetExample {
	public static void main(String[] args) {
		Set<Book1> set = new TreeSet<Book1>();
		// Creating Book1s
		Book1 b1 = new Book1(121, "Let us C", "Yashwant Kanetkar", "BPB", 8);
		Book1 b2 = new Book1(233, "Operating System", "Galvin", "Wiley", 6);
		Book1 b3 = new Book1(101, "Data Communications & Networking", "Forouzan", "Mc Graw Hill", 4);
		Book1 b4 = new Book1(190, " Networking", "Forouzan", "Hill", 6);
		Book1 b5 = new Book1(165, "Data Communications", "Rey", "Graw Hill", 4);
		Book1 b6 = new Book1(133, "Lamda Networking", "Forouzan  Ham", "Mc Graw Hill", 9);
		
		// Adding Book1s to TreeSet
		set.add(b1);
		set.add(b2);
		set.add(b3);
		set.add(b4);
		set.add(b5);
		// Traversing TreeSet
		for (Book1 b : set) {
			System.out.println(b.id + " " + b.name + " " + b.author + " " + b.publisher + " " + b.quantity);
		}
	}
}